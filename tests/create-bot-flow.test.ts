...
describe("alur adapter Telegram", () => {
  it("meneruskan foto privat secara transient tanpa menyimpan byte ke history", async () => {
    ...
    const harness = basicHarness(
      {
        ...
        reply: async (
          ...
          runtimeSeen = runtime;
          return "Warna dominannya terlihat jelas.";
        },
      } as unknown as Conversation,
      {} as TaskService,
      { telegramFilePath: "photos/fixture.jpg" },
    );

    try {
      await harness.bot.handleUpdate(photoUpdate("Warna dominannya apa?", 1));
    ...
    assert.deepEqual(harness.turns.map((turn) => turn.text), [
  ...
  it("tidak mengunduh foto Telegram sebelum consent", async () => {
    ...
    try {
      await harness.bot.handleUpdate(photoUpdate("", 1));
      await harness.bot.drainPending();
    } finally {
      globalThis.fetch = originalFetch;
    }
    ...
    assert.equal(
      harness.telegramCalls.some((call) => call.method === "getFile"),
      false,
    );
    assert.ok(harness.sent.some((text) => /kirim ulang gambarnya/iu.test(text)));
    assert.equal(harness.turns.length, 0);
  });

  it("commit task mendahului presentasi dan balasan bebas tidak boleh mengarang status", async () => {
    const text =
      "Tolong ingatkan saya satu menit lagi untuk memeriksa ketepatan waktu pengingat ini.";
    const reminderAt = new Date("2099-08-25T01:12:23.000Z");
    ...
    const harness = basicHarness(
      {
        ...
        understand: async () => understanding({
          ...
          taskAction: "save",
          task: {
            title: "Memeriksa ketepatan waktu pengingat",
            dueAt: null,
            remindAt: reminderAt,
            importance: 2,
          },
          ...
          semanticOperation: semanticOperation(
            ...
            "save",
            text,
            "memeriksa ketepatan waktu pengingat",
          ),
        }),
        triageRisk: async () => CALM_TRIAGE,
        reply: async () => {
          replyCalls += 1;
          return "Siap, pengingatnya sudah kubuat.";
        },
        presentOperation: async (
          brief: Parameters<Conversation["presentOperation"]>[0],
        ) => {
          presentationCalls += 1;
          assert.equal(taskCommitted, true);
          assert.equal(brief.kind, "task-created");
          assert.equal(brief.userMessage, text);
          return `Sudah benar-benar tersimpan.\n\n${brief.stableBody}`;
        },
      } as unknown as Conversation,
      {
        create: async (input: NewTask) => {
          assert.equal(input.remindAt?.toISOString(), reminderAt.toISOString());
          taskCommitted = true;
    ...
    assert.ok(harness.sent.some((value) =>
      value.includes("Sudah benar-benar tersimpan") &&
      value.includes("Memeriksa ketepatan waktu pengingat")
    ));
    assert.ok(!harness.sent.some((value) => value.includes("sudah kubuat")));
  });
  ...
  it("membaca task aktif dari bahasa natural tanpa AgentRun", async () => {
    const text =
      "Sekarang sebutkan tugas aktifku dan kapan pengingatnya. Jangan tampilkan ID teknis.";
    const active: StudentTask = {
      id: "task-telegram-technical",
      ownerId: "123",
      chatId: "123",
    ...
    assert.match(reply, /Meninjau hasil uji live/u);
    assert.match(reply, /10\.30/u);
    assert.match(reply, /09\.30/u);
    assert.doesNotMatch(reply, /task-telegram-technical|\bID:/u);
  });
  ...
  it("mengubah jadwal task natural secara owner-scoped tanpa AgentRun", async () => {
    const text =
    ...
    let active: StudentTask = {
      id: "task-live",
      ownerId: "123",
      chatId: "123",
      title: "Meninjau hasil uji live Telegram dan WhatsApp",
      dueAt: "2099-08-25T02:00:00.000Z",
      importance: 3,
      status: "active",
      createdAt: "2026-08-24T11:00:00.000Z",
      completedAt: null,
      reminderAt: "2099-08-25T01:30:00.000Z",
      reminderSentAt: null,
    };
    let updates = 0;
    const taskService = {
      ...
      updateSchedule: async (
        ...
        if (
          update.expected &&
          (update.expected.dueAt !== active.dueAt ||
            update.expected.reminderAt !== active.reminderAt)
        ) return null;
        updates += 1;
        active = {
          ...active,
          dueAt: update.dueAt?.toISOString() ?? active.dueAt,
          reminderAt: update.reminderAt?.toISOString() ?? active.reminderAt,
        };
        return active;
      },
    } as unknown as TaskService;
    const harness = basicHarness({
      ...
      triageRisk: async () => CALM_TRIAGE,
    } as unknown as Conversation, taskService);

    await harness.bot.handleUpdate(messageUpdate(text, 1));
    await harness.bot.drainPending();

    assert.equal(updates, 1);
    assert.equal(active.dueAt, "2099-08-25T03:30:00.000Z");
    assert.equal(active.reminderAt, "2099-08-25T02:30:00.000Z");
    assert.match(harness.sent.at(-1) ?? "", /10\.30/u);
    assert.match(harness.sent.at(-1) ?? "", /09\.30/u);
  });

  it("menyelesaikan task natural secara owner-scoped tanpa AgentRun", async () => {
    ...
    const harness = basicHarness({
      ...
      reply: async () => {
        replyCalls += 1;
        return "Ini tidak boleh dipanggil.";
      },
      presentOperation: async (
        brief: Parameters<Conversation["presentOperation"]>[0],
      ) => brief.stableBody,
    } as unknown as Conversation, {
      listActive: async () => active.status === "active" ? [active] : [],
      complete: async (ownerId: string, id: string) => {
        assert.equal(ownerId, "123");
        assert.equal(id, active.id);
        completeCalls += 1;
        active = {
    ...
    assert.match(harness.sent.at(-1) ?? "", /Mencatat hasil restart/u);
    assert.doesNotMatch(harness.sent.at(-1) ?? "", /\bID:/u);
  });
  ...
  it("menyatukan /memori, pertanyaan natural, dan Data & izin pada satu potret", async () => {
    ...
    assert.equal(portraitCompilerCalls, 0);
  ...
  it("menampilkan empty state tanpa tombol Ubah bila belum ada yang berarti", async () => {
    ...
    const harness = basicHarness(
      ...
      {} as TaskService,
      {
        ...
        histories: {
          context: async () => ({
            summary: "Sedang mengaudit tool pada percakapan aktif.",
            turns: [],
          }),
    ...
    assert.doesNotMatch(harness.sent.at(-1) ?? "", /mengaudit tool/iu);
    ...
    const harness = basicHarness(
      {
        ...
        understand: async () => understanding({
          memories: [{
            ...
            sourceEvidence: "aku lebih produktif pagi",
            sourceSubject: "self",
            durability: "durable",
          }],
        }),
        ...
        reply: async (
          ...
          assert.deepEqual(runtime.memoryAcknowledgements, [{
            operation: "updated",
            content: "aku lebih produktif pagi",
            explicit: false,
          }]);
          return naturalReply;
        },
  ...
  it("tidak mengaku aturan dicatat ketika write memori tidak punya receipt", async () => {
    ...
    await harness.bot.handleUpdate(messageUpdate(
      "Kalau membantu pekerjaan produk, jawab dengan keputusan utama dulu.",
    ));
    await harness.bot.drainPending();

    assert.doesNotMatch(harness.sent.join("\n"), /aturan baru dicatat/iu);
  });

  it("tidak menyisipkan ajakan dukungan setelah balasan kerja biasa", async () => {
    let supportCalls = 0;
    const harness = basicHarness(
      {
        classifyTurnBoundary: async () => "complete",
        understand: async () => understanding(),
        triageRisk: async () => CALM_TRIAGE,
        reply: async () => "Artefak final persis empat baris.",
      } as unknown as Conversation,
      {} as TaskService,
      {
        economy: {
          supportPrompt: async () => {
            supportCalls += 1;
            return "Ajakan dukungan yang tidak diminta.";
          },
    ...
    await harness.bot.handleUpdate(messageUpdate(
      "Berikan artefak final persis empat baris dan jangan tambahkan apa pun.",
    ));
    await harness.bot.drainPending();

    assert.equal(supportCalls, 0);
    assert.deepEqual(harness.sent, ["Artefak final persis empat baris."]);
  });
    ...
    const harness = basicHarness(
      {
        ...
        understand: async () => understanding({
          memories: [
            {
              kind: "profile",
              content: "Nama panggilan Hafizh",
              sourceEvidence: "panggil aku Hafizh",
              sourceSubject: "self",
              durability: "durable",
            },
            {
              kind: "preference",
              content: "Lebih nyaman belajar pagi",
              sourceEvidence: "aku juga lebih nyaman belajar pagi",
              sourceSubject: "self",
              durability: "durable",
            },
          ],
  ...
  it("tetap menyimpan preferensi scoped ketika turn masuk jalur planning", async () => {
    const saved: NewMemory[] = [];
    const text =
      "Mulai sekarang, kalau membantu pekerjaan produk, jawab dengan keputusan utama dulu lalu alasan singkat; jangan buka dengan empati generik. Terapkan mulai jawaban berikutnya.";
    const harness = basicHarness(
      {
        classifyTurnBoundary: async () => "complete",
        understand: async () => understanding({
        ...
        agent: async () => ({
          status: "completed",
          reply: "Keputusan utama: format itu berlaku mulai jawaban berikutnya.",
        }),
      } as unknown as Conversation,
      {} as TaskService,
      {
        memories: {
          ...
          remember: async (input: NewMemory) => {
            saved.push(input);
            const item = memoryItem("scoped-product-style", input.content);
            item.kind = input.kind;
            return item;
          },
          markUsed: async () => undefined,
    ...
    assert.equal(saved.length, 1);
    assert.match(saved[0]?.content ?? "", /pekerjaan produk.*keputusan utama/iu);
    assert.doesNotMatch(harness.sent.join("\n"), /SIMPAN MEMORI|JANGAN SIMPAN/iu);
  });
        ...
        understand: async () => understanding({
          ...
          memories: [
            { kind: "personal", content: "Sohit adalah pacarku" },
            {
              ...
              sourceEvidence: "tadi aku habis dari rumah sakit",
              sourceSubject: "self",
              durability: "bounded",
            },
          ],
    ...
    const harness = basicHarness(
      {
        ...
        presentOperation: async (
          brief: Parameters<Conversation["presentOperation"]>[0],
        ) => `Oke, aku catat setelah penyimpanannya berhasil.\n\n${brief.stableBody}`,
        reviewReply: async () => {
          reviews += 1;
          return true;
        },
    ...
    const harness = basicHarness(
      {
        ...
        presentOperation: async (
          brief: Parameters<Conversation["presentOperation"]>[0],
        ) => `Kedengarannya lagi berat. Tugasnya tetap aku catat.\n\n${brief.stableBody}`,
        reviewReply: async () => {
          reviews += 1;
          return true;
        },
  ...
  it("aritmetika sederhana memberi hasil exact tanpa model di Telegram", async () => {
    ...
    await harness.bot.handleUpdate(
      messageUpdate("Sekarang jawab 17+28 dengan angka saja."),
    );
    await harness.bot.drainPending();

    assert.equal(understandingCalls, 0);
    assert.equal(replies, 0);
    assert.equal(harness.sent.at(-1), "45");
  });
  ...
  it("memisahkan auto-memory privat dari acute-safety routing", async () => {
    ...
    const harness = basicHarness(
      {
        ...
        understand: async () => understanding({
          memories: [{
            ...
            sourceEvidence: "aku kesulitan membaca soal panjang saat ujian",
            sourceSubject: "self",
            durability: "bounded",
          }],
        }),
    ...
    assert.equal(
      storedContent,
      "aku kesulitan membaca soal panjang saat ujian",
      "parafrasa model tidak boleh menjadi canonical primary memory",
    );
    assert.match(harness.sent.at(-1) ?? "", /ingat untuk ke depan|📍/iu);
    assert.doesNotMatch(harness.sent.join("\n"), /Boleh aku inget/iu);
  });

  it("mencabut ingatan lama dan menyimpan koreksi grounded dalam satu turn Telegram", async () => {
    const text = [
    ...
    const items: MemoryItem[] = [
      memoryItem("old-english", "Prefers coding conversations in English"),
      memoryItem("old-garden", "Memiliki kebun kecil"),
    ];
    ...
    const harness = basicHarness(
      {
        ...
        reply: async (
          ...
          replyContext = context.memories.map((item) => item.content);
          return "Oke, dua ingatan lama itu sudah aku hapus. Pola penjelasanmu akan kuingat.";
        },
      } as unknown as Conversation,
      {} as TaskService,
      {
        memories: {
          relevantTo: async () => [...items],
          list: async () => [...items],
          forget: async (_ownerId: string, id: string) => {
            const index = items.findIndex((item) => item.id === id);
            return index >= 0 ? items.splice(index, 1)[0] ?? null : null;
          },
          remember: async (input: NewMemory) => {
            const item = memoryItem("new-core-first", input.content);
            items.push(item);
            return item;
          },
          markUsed: async () => undefined,
    ...
    assert.deepEqual(replyContext, [], "source yang dicabut tidak boleh ikut prompt reply");
    assert.match(harness.sent.at(-1) ?? "", /sudah aku hapus/iu);
  });
  ...
  it("menolak topik kerja dan keadaan sekali pakai sebagai auto-memory Telegram", async () => {
    const text =
    ...
    const harness = basicHarness(
      ...
      {} as TaskService,
      {
        memories: {
          ...
          remember: async () => {
            remembers += 1;
            return memoryItem("must-not-save", "Tidak boleh tersimpan");
          },
        } as unknown as MemoryService,
      },
    );
  ...
  it("menjawab pertanyaan biasa tanpa tool dan mengikat settlement ke turn", async () => {
    let agentCalls = 0;
    let replyCalls = 0;
    let deliveredTurn: string | null | undefined;
    let replyRuntime: ConversationRuntime | null = null;
    const publicFocus = {
    ...
    const harness = basicHarness(
      {
        ...
        agent: async () => {
          agentCalls += 1;
          throw new Error("pertanyaan tanpa tool tidak boleh membuka agent");
        },
        reply: async (...args: unknown[]) => {
          replyCalls += 1;
          replyRuntime = args[7] as ConversationRuntime;
    ...
    assert.deepEqual(
      (replyRuntime as ConversationRuntime | null)?.publicProgressFocus,
      publicFocus,
    );
  });
  ...
  it("request pendek bernuansa tanpa tool tetap memakai reply", async () => {
    let agentCalls = 0;
    let replyCalls = 0;
    const harness = basicHarness({
      classifyTurnBoundary: async () => "complete",
      ...
      agent: async () => {
        agentCalls += 1;
        throw new Error("nuansa tinggi bukan authority tool");
      },
      reply: async () => {
        replyCalls += 1;
        return "Kita timbang pilihanmu pelan-pelan.";
      },
    } as unknown as Conversation);
    ...
    assert.equal(agentCalls, 0);
    assert.equal(replyCalls, 1);
    assert.match(harness.sent.at(-1) ?? "", /timbang pilihanmu/iu);
  });
  ...
  it("menjaga curhat tentang pekerjaan rumit di chat lane meski model meminta planning", async () => {
    let agentCalls = 0;
    let replyCalls = 0;
    const harness = basicHarness({
      classifyTurnBoundary: async () => "complete",
      ...
      reply: async () => {
        replyCalls += 1;
        return "Yang berat itu catatannya berantakan dan takut salah memilih. Kita urai satu-satu.";
      },
    } as unknown as Conversation);
    ...
    assert.equal(agentCalls, 0);
    assert.equal(replyCalls, 1);
    assert.ok(harness.sent.some((text) => /kita urai satu-satu/iu.test(text)));
  });

  it("menjawab permintaan analisis di chat ketika planning tidak membutuhkan tool", async () => {
    let agentCalls = 0;
    let replyCalls = 0;
    const harness = basicHarness({
      classifyTurnBoundary: async () => "complete",
      ...
      reply: async () => {
        replyCalls += 1;
        return "Tool salah harus diprioritaskan karena dapat merusak kepercayaan.";
      },
    } as unknown as Conversation);
    ...
    assert.equal(agentCalls, 0);
    assert.equal(replyCalls, 1);
    assert.ok(harness.sent.some((text) => /merusak kepercayaan/iu.test(text)));
    assert.equal(
      harness.sent.some((text) => /Menunggu giliran kerja/iu.test(text)),
      false,
      JSON.stringify(harness.sent),
    );
  });

  it("tetap menuntaskan permintaan utama saat model salah mengusulkan remember pada negasi", async () => {
    const message =
    ...
    assert.equal(saves, 0);
    assert.equal(replyCalls, 1);
    assert.match(harness.sent.at(-1) ?? "", /tool salah, onboarding/iu);
    assert.doesNotMatch(harness.sent.join("\n"), /udah aku hapus/iu);
    assert.doesNotMatch(harness.sent.join("\n"), /sudah aku catat/iu);
    assert.doesNotMatch(
      harness.sent.join("\n"),
  ...
  it("tidak menambahkan tawaran tugas ketika balasan sudah meminta klarifikasi", async () => {
    const harness = basicHarness({
      classifyTurnBoundary: async () => "complete",
      triageRisk: async () => CALM_TRIAGE,
      understand: async () => understanding({
        ...
        taskAction: "offer",
        task: {
          title: "Memilih tiga temuan produk terpenting",
          dueAt: null,
          remindAt: null,
          importance: 2,
        },
      ...
      reply: async () =>
        "Kita urai pelan-pelan. Temuannya paling banyak membahas bagian apa?",
    } as unknown as Conversation);

    await harness.bot.handleUpdate(messageUpdate(
      "Aku kewalahan membaca temuan produk dan belum tahu tiga yang paling penting.",
    ));
    await harness.bot.drainPending();

    assert.match(harness.sent.at(-1) ?? "", /membahas bagian apa\?/iu);
    assert.doesNotMatch(harness.sent.join("\n"), /Mau aku catat/iu);
    assert.equal(findCallbacks(harness.telegramCalls, "save:").length, 0);
  });

  it("tidak menyisipkan survei gaya saat permintaan sedang menunggu jawaban", async () => {
    const priorTurns: ConversationTurn[] = Array.from(
      { length: HISTORY_WINDOW },
      (_, index) => ({
        role: index % 2 === 0 ? "user" as const : "harvy" as const,
        text: `giliran ${index + 1}`,
        at: new Date().toISOString(),
      }),
    );
    const profileValue = profile({
    ...
    const harness = basicHarness(
      {
        ...
        reply: async () => "Catatannya sekarang masih campur atau sudah dikelompokkan?",
      } as unknown as Conversation,
      {} as TaskService,
      {
        profiles: {
          ...
          markStyleAsked: async () => {
            styleAsked += 1;
            profileValue.styleAskedAt = new Date().toISOString();
            return profileValue;
          },
        } as unknown as ProfileService,
        histories: {
          context: async () => ({ summary: null, turns: [...priorTurns] }),
          append: async () => undefined,
          compact: async () => undefined,
          allow: () => undefined,
          suspend: () => undefined,
    ...
    await harness.bot.handleUpdate(messageUpdate(
      "Untuk pekerjaan ini, bantu lewat percakapan saja dan tanyakan satu hal.",
    ));
    await harness.bot.drainPending();

    assert.equal(styleAsked, 0);
    assert.equal(harness.sent.some((text) => text === STYLE_QUESTION), false);
  });
    ...
    const harness = basicHarness(
      {
        classifyTurnBoundary: async () => "complete",
        triageRisk: async () => CALM_TRIAGE,
        understand: async () => toolQuestionUnderstanding(),
        agent: async (
          _request: string,
          _mode: string,
          _context: HarvyContext,
    ...
    const harness = basicHarness(
      {
        ...
        understand: async (message: string) =>
          understanding({
            ...
            routingAssessment: message.includes("analisis")
              ? routingAssessment({ toolNeed: "external" })
              : routingAssessment(),
          }),
        agent: async (
          _request: string,
  ...
  it("/menu deterministik, category-based, dan berbeda dari /bantuan", async () => {
    ...
    assert.ok(findCallback(harness.telegramCalls, "menu:memory"));
    ...
    await harness.bot.handleUpdate(callbackUpdate("menu:memory", 3));
    await harness.bot.drainPending();
    const memoryEdit = harness.telegramCalls.find((call) =>
      call.method === "editMessageText" &&
      /^Memori & data/u.test((call.payload as { text?: string }).text ?? "")
    );
    assert.ok(memoryEdit);
    const memoryCallbacks = (
      (memoryEdit.payload as {
        reply_markup?: {
          inline_keyboard?: Array<Array<{ callback_data?: string }>>;
        };
      }).reply_markup?.inline_keyboard ?? []
    ).flat().map((button) => button.callback_data);
    assert.ok(memoryCallbacks.includes("control:delete-all"));

    await harness.bot.handleUpdate(callbackUpdate("menu:settings", 4));
    await harness.bot.drainPending();
    ...
    await harness.bot.handleUpdate(commandUpdate("/bantuan", 5));
    await harness.bot.drainPending();
  ...
  it("/hapus-data membuka konfirmasi bertoken yang sama tanpa memanggil model", async () => {
    let modelCalls = 0;
    let deletions = 0;
    const forbidden = (): never => {
      modelCalls += 1;
      throw new Error("hapus data exact tidak boleh memanggil model");
    };
    const harness = basicHarness(
      {
        classifyTurnBoundary: async () => forbidden(),
      ...
      {
        dataControls: {
          deleteAll: async (ownerId: string) => {
            assert.equal(ownerId, "123");
            deletions += 1;
          },
        } as unknown as DataControlService,
    ...
    const update = commandUpdate("/hapus-data", 1);
    if (update.message?.entities?.[0]) {
      // Telegram membatasi entity command pada bagian sebelum tanda hubung.
      update.message.entities[0].length = "/hapus".length;
    }
    ...
    assert.equal(deletions, 0);
    assert.equal(modelCalls, 0);
    assert.match(harness.sent.at(-1) ?? "", /tidak bisa dibatalkan/iu);
    const confirmation = findCallbacks(harness.telegramCalls, "datawipe:");
    assert.equal(confirmation.length, 2);
    const yes = confirmation.find((callback) => callback.endsWith(".yes"));
    assert.ok(yes);

    await harness.bot.handleUpdate(callbackUpdate(yes, 2));
    await harness.bot.drainPending();
  ...
  it("tidak membiarkan proposal usage explicit membajak penilaian produk nonmekanis", async () => {
    const text =
      "Menurutmu, apakah pengujian live yang kita jalankan sudah cukup untuk membuktikan Harvy siap dipakai sehari-hari? Nilai bukti dan celah yang masih berisiko.";
    let summaryCalls = 0;
    let agentCalls = 0;
    let replyCalls = 0;
    const harness = basicHarness({
...
describe("work lane active AgentRun Telegram", () => {
  ...
  it("menjaga chat lane responsif saat pekerjaan durable masih berjalan", async () => {
    ...
    const request =
      "Aku biasanya paling fokus belajar pagi. Tolong buatkan rencana belajar langkah demi langkah.";
    const started = deferredVoid();
    const release = deferredVoid();
    let agentCalls = 0;
    let replyCalls = 0;
    const harness = basicHarness(
      ...
      {} as TaskService,
      {
        ...
        memories: {
          relevantTo: async () => [],
          remember: async (input: NewMemory) => {
            const item = memoryItem("active-run-morning", input.content);
            item.kind = input.kind;
            return item;
          },
          markUsed: async () => undefined,
    ...
    try {
      ...
      const noticeIndex = harness.sent.findIndex((text) =>
        /ingat.*ke depan|📍/iu.test(text)
      );
      const anchorIndex = harness.sent.findIndex((text) => text.startsWith("📌 "));
      assert.ok(noticeIndex >= 0);
      assert.equal(anchorIndex, noticeIndex + 1);
      assert.doesNotMatch(harness.sent[anchorIndex] ?? "", /ingat.*ke depan|📍/iu);
  ...
  it("menahan commit ketika reply ke Anchor masih melewati safety", async () => {
    const root = await mkdtemp(join(tmpdir(), "harvy-active-run-ingress-race-"));
    const runs = new AgentRunService(
      new FileAgentRunRepository(join(root, "agent-runs.json")),
    );
    ...
    try {
      ...
      await harness.bot.handleUpdate(replyMessageUpdate(correction, 2, anchorId));
      await correctionUnderstandingStarted.promise;
      releaseFirst.resolve();
      await new Promise((resolve) => setTimeout(resolve, 30));
      assert.equal(
        harness.sent.includes("HASIL LAMA YANG TIDAK BOLEH TERKIRIM"),
        false,
      );
      ...
      assert.ok(harness.sent.includes("Rencana terkoreksi tanpa langkah publikasi."));
      assert.equal(
        harness.sent.filter((text) => text.startsWith("📌 ")).length,
        1,
      );
    } finally {
      releaseFirst.resolve();
...
function photoUpdate(caption: string, updateId = 1): Update {
  ...
  if (update.message) {
    ...
    update.message.photo = [{
      ...
      file_unique_id: `unique-photo-${updateId}`,
      width: 32,
      height: 32,
      file_size: 8,
    }];
...
function installFakeTelegram(
  ...
  bot.api.config.use((async (_previous, method, payload) => {
    ...
    if (method === "getFile" && telegramFilePath) {
      const body = payload as { file_id?: string };
      return {
        ok: true,
        result: {
          file_id: body.file_id ?? "photo-fixture",
          file_unique_id: "photo-fixture-unique",
          file_size: 8,
...
function toolQuestionUnderstanding() {
  return understanding({
    intent: "question",
    routingAssessment: routingAssessment({
      toolNeed: "external",
