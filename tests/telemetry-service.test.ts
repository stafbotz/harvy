...
describe("token berjalan per giliran", () => {
  it("menjumlahkan setiap panggilan model dalam giliran yang sama", async () => {
    ...
    assert.deepEqual(telemetry.turnTokens("siswa", "giliran-1"), { input: 1, output: 1, pendingInput: 0 });
    await observeModelCall(telemetry, "siswa", "giliran-1", "reply", "r2");
    assert.deepEqual(telemetry.turnTokens("siswa", "giliran-1"), { input: 2, output: 2, pendingInput: 0 });
  });
  ...
  it("tidak berkurang ketika antrean telemetry dikuras ke penyimpanan", async () => {
    ...
    assert.deepEqual(sebelum, { input: 1, output: 1, pendingInput: 0 });
  });
  ...
  it("memisahkan giliran dan pengguna", async () => {
    ...
    assert.deepEqual(telemetry.turnTokens("siswa", "giliran-1"), { input: 1, output: 1, pendingInput: 0 });
    assert.deepEqual(telemetry.turnTokens("siswa", "giliran-2"), { input: 1, output: 1, pendingInput: 0 });
    assert.deepEqual(telemetry.turnTokens("lain", "giliran-1"), { input: 1, output: 1, pendingInput: 0 });
  });
  ...
  it("mengembalikan nol tanpa identitas giliran", async () => {
    ...
    assert.deepEqual(telemetry.turnTokens("siswa", null), { input: 0, output: 0, pendingInput: 0 });
    assert.deepEqual(telemetry.turnTokens("siswa", "giliran-lain"), { input: 0, output: 0, pendingInput: 0 });
    assert.deepEqual(telemetry.turnTokens("bukan-siapa-siapa", "giliran-1"), { input: 0, output: 0, pendingInput: 0 });
  });
  ...
  it("melepas penghitung setelah giliran selesai", async () => {
    ...
    assert.deepEqual(telemetry.turnTokens("siswa", "giliran-1"), { input: 0, output: 0, pendingInput: 0 });
