...
membedakan balasan terpotong dari balasan rusak. Perlu `.env` berisi key GMI
sungguhan saat `AI_MODE=testing`. Probe ini memanggil model dan dapat berbiaya,
jadi tidak boleh masuk gerbang otomatis. Semua probe primary-only; runtime
tidak mempunyai provider fallback. Runtime, probe, dan evaluator mengambil lock
...
wajib loopback dan bukan server internet-ready. Mode testing memakai
`GMI_API_KEY`, `AI_MODEL_TESTING`, dan base URL GMI bawaan; daftar environment
provider testing lama dapat dibersihkan sekali lewat
`npm run ai:testing:migrate-gmi`. WhatsApp beta memakai `WHATSAPP_ENABLED`,
...
ID model aktif dan harga dibaca dari environment agar koreksi tidak memerlukan
perubahan business policy. `AI_MODE=testing` memakai satu endpoint
OpenAI-compatible GMI Serving dengan `GMI_API_KEY`; target lokal saat ini adalah
`MiniMaxAI/MiniMax-M3`. `AI_MODE=production` memakai tiga model lewat
OpenRouter. Kedua mode tidak mempunyai provider fallback dan tanpa key aktif
bot menolak start.

Harvy Console menginventarisasi seluruh slot model environment yang nonkosong
pada startup—testing default/override dan production—tetapi hanya
...
Base URL GMI dikonfigurasi tanpa suffix `/chat/completions`; klien Harvy
menambahkannya sendiri. Key dikirim lewat `Authorization: Bearer` dan model
melalui body OpenAI-compatible. Retry terbatas tetap berada pada provider GMI
yang sama; konfigurasi saat ini hanya memasang satu key dan tidak pernah
memindahkan request ke provider lain. Evaluator dan probe model nyata selalu
mencatat `fallbackAllowed: false` serta
...
diverifikasi operator bagi pasangan provider+ID model exact. Tanpa deklarasi
environment atau profile code-owned live-proven, slot memakai profile
`compatibility`: routing/tool behavior lama tetap ada, tetapi Harvy tidak
mengirim reasoning control atau replay reasoning assistant-level baru. Exact
endpoint resmi GMI + MiniMax-M3 mempunyai profile code-owned dari smoke 25
Agustus 2026; custom gateway/model lain tidak mewarisinya. Base URL atau
substring nama model tidak pernah dipakai untuk menebak capability.
...
tersedia saat ini: `openai-reasoning-effort` untuk profile OpenAI-compatible
yang diverifikasi explicit, `openrouter-reasoning`, `deepseek-thinking`, atau
`none`. GMI/MiniMax tetap memakai `none` sampai smoke exact lulus. DeepSeek baru
...
Composition runtime tidak mempunyai provider fallback; profile exact hanya
berlaku pada primary provider+model yang terdaftar.
