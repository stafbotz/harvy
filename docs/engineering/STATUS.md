...
- Bukti baseline: `npm run check` PASS; `npm test` PASS, 1.120 test dalam 138
...
| WhatsApp grup | Fondasi beta, adaptive batching/selective ingress, lifecycle guard, serta core Group AgentRun group-safe dengan explicit thread/authority/CAS ada | Group AgentRun belum wiring kanal/work lane/outbox; perilaku lengkap dan dua nomor nyata belum diuji | 2026-08-13 | [`status/whatsapp.md`](status/whatsapp.md) |
...
| Project & coding | Safe ProjectWorkspace, tombstone-first deletion saga + worker recovery scope-free, durable sandbox/evidence policy dengan startup/shutdown fence eksplisit, CodingRun map/plan/task-review single-writer, immediate scheduler CAS+quiescence, maintenance supervisor admission-closed, private-audience local-git/GitHub exact-effect, worker observasi receipt ambigu, dan client HTTP trust-domain dengan service-auth proof ada default-off | Belum ada runner Linux, daemon local-git/object store, GitHub App broker, secret+verifier conformance live, Workspace ingress/driver/composition, wiring startup supervisor, atau metadata project/run/evidence/deletion/GitHub multi-instance | 2026-08-13 | [`status/coding.md`](status/coding.md) |
