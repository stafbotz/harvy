...
- Basis: working tree 5 Agustus 2026 yang belum di-commit, termasuk Harvy Loop,
...
- Cara verifikasi: membaca kode secara langsung dan menjalankan gerbang
  otomatis (`npm run check` dan `npm test`: 639 test dalam 93 suite), probe
...
| Agent Runtime privat | Ada dengan native tool calling, teruji otomatis; probe model lama masih memakai build sebelum perubahan ini, persistence restart teruji, Telegram nyata belum | Setelah consent dan triase tenang+pasti, intent `question`/`request`, permintaan planning eksplisit, serta query state-live presisi tanpa sesi aktif masuk root agent. Pekerjaan sederhana/tool memakai root `cheap`; `needsStepByStep`, planning eksplisit, atau pesan di atas 280 karakter memakai root `ambitious`. Setiap langkah planner mengirim `tools`, `tool_choice: required`, dan `parallel_tool_calls: false`; `final` dan `need_input` juga menjadi function tertutup. Plain text, function asing, argumen rusak, atau lebih dari satu call ditolak sebelum keputusan masuk kernel. Pass pertama ambitious tidak menerima konteks privat dan hanya boleh fan-out; sintesis/alternatif memakai konteks terpilih dengan turn berperan chat. `need_input` disimpan owner-scoped setelah seluruh prompt terkirim, dapat dipulihkan setelah restart normal, dan dilanjutkan pada checkpoint yang sama selama horizon absolut 10 menit; setiap invocation aktif dibatasi 45 detik. Record memakai CAS, watermark update Telegram, codec checkpoint bersama, worker expiry, ekspor/penghapusan, dan consent v6. Frasa personal berpresisi tinggi tentang tugas/sesi/waktu/agenda wajib ditopang observation live beserta cakupan/tanggal lokal yang cukup dan menang atas salah-intent operational model, tetapi pagar regex ini belum universal untuk semua parafrasa. Safety, consent, kontrol data, dan mutasi tugas/memori/sesi tetap route terpisah. Native request sengaja primary-only sampai kompatibilitas fallback dibuktikan. Model/provider nyata dan Telegram staging belum dijalankan untuk perubahan native ini |
...
- Capability catalog, kernel agent, native tool calling, tool baca state
  ...
  Native planner baru dibuktikan dengan provider palsu; belum ada smoke model
  primary atau Telegram staging, dan fallback native sengaja dinonaktifkan
  sampai provider cadangan terbukti kompatibel. Belum ada RunStore produksi
