...
- Basis: commit `fc88565` tanggal 6 Agustus 2026, termasuk Harvy Loop,
  ...
  `ADR-013`, structured episodic compaction v2 `ADR-014`, pencabutan executor
  web baca-saja `ADR-015`, Scope & Authority v1 `ADR-016`, serta Agent
  Runtime internal dan delegasi berbatas `ADR-017`, checkpoint klarifikasi
  durable lokal `ADR-018`, serta native tool calling planner agent.
- Cara verifikasi: membaca kode secara langsung dan menjalankan gerbang
  otomatis (`npm run check` dan `npm test`: 644 test dalam 93 suite), probe
...
| Capability awareness dan kernel agent | Ada dengan native planner, executor internal baca-saja, dan checkpoint klarifikasi durable lokal; RunStore produksi belum ada | Satu catalog tepercaya menghasilkan snapshot ter-hash dan immutable per scope/surface lalu masuk ke prompt privat maupun grup. Planner action hanya melihat `callableCapabilities`: irisan entry available dengan executor+versi yang benar-benar terpasang, sehingga fitur deterministik seperti `task.manage` tidak menyamar sebagai tool. Setiap executor callable memiliki nama+JSON Schema native tanpa titik; metadata itu dibekukan dan ikut hash checkpoint. Scope bertipe mencakup private, group, dan workspace; Workspace tetap tidak aktif. Kernel tetap memvalidasi hasil normalisasi `final/need_input/action`, versi capability, input, approval exact-value, idempotency, cycle, cancellation, generation/ACL freshness, dan checkpoint sebelum executor berjalan. Checkpoint membekukan `startedAt`, horizon resume absolut, `maxSteps`, serta hash capability+executor+kontrak native; perubahan authority menghentikan resume. Hanya status `waiting_input` privat Telegram yang disimpan lewat adapter file satu-proses; run aktif/Workspace belum durable. Run Workspace wajib revalidator authority |
| Agent Runtime privat | Ada dengan native tool calling, teruji otomatis; satu insiden primary+Telegram nyata ditemukan sebelum perbaikan, post-fix belum diuji live | Setelah consent dan triase tenang+pasti, intent `question`/`request`, permintaan planning eksplisit, serta query state-live presisi tanpa sesi aktif masuk root agent. Pekerjaan sederhana/tool memakai root `cheap`; `needsStepByStep`, planning eksplisit, atau pesan di atas 280 karakter memakai root `ambitious`. Setiap langkah planner mengirim `tools`, `tool_choice` required atau nama function yang tersedia, dan `parallel_tool_calls: false`; `final` dan `need_input` juga menjadi function tertutup. Plain text, function asing, argumen rusak, balasan control kosong, atau lebih dari satu call ditolak sebelum keputusan masuk kernel. Dalam satu invocation, continuation menyimpan transcript provider ephemeral dan memutar ulang exact assistant `tool_calls`, pesan `tool` dengan call ID cocok, serta thought signature Gemini; semuanya dibuang ketika invocation berakhir dan tidak masuk checkpoint. Kebutuhan live-state memilih function yang diperlukan sebelum inference; sesudah observation, planner tetap boleh memilih final atau tool lain sehingga rencana multi-tool tidak diterminasi dini. Pass pertama ambitious tidak menerima konteks privat dan hanya boleh fan-out; sintesis/alternatif memakai konteks terpilih dengan turn berperan chat. `need_input` disimpan owner-scoped setelah seluruh prompt terkirim, dapat dipulihkan setelah restart normal, dan dilanjutkan pada checkpoint yang sama selama horizon absolut 10 menit; prompt yang dijawab dipasangkan dengan jawaban di checkpoint provider-neutral agar referen pendek tetap tersedia. Setiap invocation aktif dibatasi 45 detik. Record memakai CAS, watermark update Telegram, codec checkpoint bersama, worker expiry, ekspor/penghapusan, dan consent v6. Frasa personal berpresisi tinggi tentang tugas/sesi/waktu/agenda wajib ditopang observation live beserta cakupan/tanggal lokal yang cukup dan menang atas salah-intent operational model, tetapi pagar regex ini belum universal untuk semua parafrasa. Safety, consent, kontrol data, dan mutasi tugas/memori/sesi tetap route terpisah. Native request sengaja primary-only sampai kompatibilitas fallback dibuktikan. Pada insiden 6 Agustus, primary menerima native calls tetapi loop berhenti sebelum reply; perbaikan sesudahnya baru dibuktikan otomatis, belum lewat primary atau Telegram |
| Tool internal agent | Ada, read-only, memakai native function schema executor, teruji otomatis; belum diuji Telegram nyata | `task.list_active`, `task.get`, `session.status`, `settings.time.get`, dan `calendar.agenda` memakai schema tertutup yang dimiliki executor serta owner dari scope tepercaya. Output task/session yang berasal dari pengguna ditandai tak tepercaya. Tool tidak dapat membuat, mengubah, menyelesaikan, atau menghapus state |
...
| Balasan tahu waktu | Ada, teruji otomatis dan adapter palsu; percobaan Telegram pre-fix gagal, post-fix belum diuji live | `replyPrompt` menerima jam dan zona waktu. Pertanyaan jam/tanggal yang berdiri sendiri memakai fast path clock runtime deterministik tanpa planner; closed set menerima tepat satu vokatif `harvy` di tepi awal atau akhir. Rencana agent dapat memakai `settings.time.get`. Tes pada instant UTC yang sama memeriksa tanggal/jam WIB versus WIT, dan adapter mengambil zona dari profil |
...
- Capability catalog, kernel agent, native tool calling, tool baca state
  internal, terminal virtual, dan delegasi read-only sudah ada untuk privat
  Telegram. Executor web dan konektor aplikasi eksternal belum terpasang.
  Satu percobaan primary lewat Telegram pada 6 Agustus menerima native calls,
  tetapi build sebelum perbaikan mengulang langkah dan berhenti sebelum reply.
  Perbaikan continuation dan fast path baru dibuktikan otomatis; belum ada
  smoke post-fix primary atau Telegram staging, dan fallback native sengaja
  dinonaktifkan sampai provider cadangan terbukti kompatibel. Belum ada RunStore produksi
  untuk run aktif, outbox, receipt, atau reconciliation
  worker. Checkpoint `waiting_input` privat sudah tahan restart normal pada
  adapter file satu-proses. Harvy tidak
...
- Harvy Loop, Agent Runtime internal/delegasi, kontrol data, preferensi waktu,
  ...
  Percobaan Telegram operasional 6 Agustus hanya membuktikan primary menerima
  native calls pada build pre-fix; run berhenti sebelum balasan dan bukan PASS
  acceptance end-to-end.
- Checkpoint `need_input` privat Telegram dipulihkan dari file setelah restart
  normal, terikat owner/run/revision/capability/executor serta horizon absolut
  sepuluh menit. Hanya batch yang seluruhnya masuk setelah prompt boleh
  menjawab; expiry dibersihkan saat startup/akses dan oleh worker satu-menit.
  Delivery Telegram dan commit file tetap tidak atomik: crash tepat setelah
  prompt diterima tetapi sebelum save masih kehilangan checkpoint. Bila commit
  pasca-delivery dan cleanup fallback sama-sama gagal, block hanya hidup dalam
  proses; restart di dalam horizon dapat melihat record lama. Belum ada
  abandonment marker durable, outbox, receipt, atau UX Telegram nyata untuk
  kegagalan ini. Run aktif juga belum dipulihkan.
