...
|---|---|---|---|---|
| Agent Runtime | Planner native, tool tulis `task.manage`/`reminder.schedule`, koreksi bentuk tool call, penjelasan penghentian, provider-aware continuation, cumulative RunBudget, context-pressure recovery, active AgentRun durable lokal, dan validator-driven `toughest` ada | Jalur agentic tunggal `tool_choice: auto` belum dirangkai; target `toughest`, CodingRun/channel live, dan RunStore distributed belum live/ada | 2026-08-28 | [`status/agent-runtime.md`](status/agent-runtime.md) |
