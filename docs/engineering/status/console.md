...
- Tab Kanal memverifikasi token bot Telegram, memasangkan session tester
  ...
  payload QR.
- Tab Kanal membedakan konfigurasi produk utama dan credential acceptance.
...
- Surface kanal mempunyai state setup dan operasional yang berbeda. Saat belum
  ...
  Setelah empat identitas tersedia dan kedua session WhatsApp diterima dalam
  pemeriksaan terbaru, layar utama menunjukkan ringkasan `Harvy siap diuji`
  dan alur tester → Harvy untuk Telegram/WhatsApp; form
...
- `npm run console:setup` mengatasi bootstrap saat runtime utama belum memiliki
  ...
  lokal berbeda yang diabaikan Git.
...
- Browser Edge desktop/mobile membuktikan transisi sesi valid→ditolak→valid,
  ...
  internal. Probe credential acceptance nyata current build membuktikan Harvy A
  `configured=true` tetapi `rejected`, sedangkan tester B `accepted`; karena itu
  A memang perlu dipasangkan ulang sebelum journey B→A, dan Console sekarang
  menyatakan fakta tersebut dengan benar.
