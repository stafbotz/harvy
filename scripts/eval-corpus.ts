...
export const CONVERSATION_EVAL_CASES: readonly ConversationEvalCase[] = [
  ...
  { id: "code-romawi", message: "buatkan fungsi JavaScript bernama persis keRomawi yang mengubah bilangan bulat 1 sampai 3999 menjadi angka Romawi. Tulis kodenya saja.", expectedIntent: "request", expectedRisk: "biasa", allowCode: true, codeCheck: { symbol: "keRomawi", assertions: [
    "assert.equal(keRomawi(4), 'IV');",
    "assert.equal(keRomawi(9), 'IX');",
    "assert.equal(keRomawi(14), 'XIV');",
    "assert.equal(keRomawi(40), 'XL');",
    "assert.equal(keRomawi(1994), 'MCMXCIV');",
    "assert.equal(keRomawi(3999), 'MMMCMXCIX');",
  ].join(String.fromCharCode(10)) } },
  { id: "code-kabisat", message: "buatkan fungsi JavaScript bernama persis hariDalamBulan yang mengembalikan jumlah hari untuk bulan dan tahun tertentu, dengan aturan tahun kabisat yang benar. Tulis kodenya saja.", expectedIntent: "request", expectedRisk: "biasa", allowCode: true, codeCheck: { symbol: "hariDalamBulan", assertions: [
    "assert.equal(hariDalamBulan(2, 2024), 29);",
    "assert.equal(hariDalamBulan(2, 2023), 28);",
    "assert.equal(hariDalamBulan(2, 1900), 28);",
    "assert.equal(hariDalamBulan(2, 2000), 29);",
    "assert.equal(hariDalamBulan(4, 2023), 30);",
    "assert.equal(hariDalamBulan(12, 2023), 31);",
  ].join(String.fromCharCode(10)) } },
  { id: "code-biner", message: "buatkan fungsi JavaScript bernama persis cariIndeks yang mencari nilai dalam array terurut dengan pencarian biner dan mengembalikan indeksnya, atau -1 bila tidak ada. Tulis kodenya saja.", expectedIntent: "request", expectedRisk: "biasa", allowCode: true, codeCheck: { symbol: "cariIndeks", assertions: [
    "assert.equal(cariIndeks([1, 3, 5, 7, 9], 7), 3);",
    "assert.equal(cariIndeks([1, 3, 5, 7, 9], 1), 0);",
    "assert.equal(cariIndeks([1, 3, 5, 7, 9], 9), 4);",
    "assert.equal(cariIndeks([1, 3, 5, 7, 9], 4), -1);",
    "assert.equal(cariIndeks([], 1), -1);",
    "assert.equal(cariIndeks([2], 2), 0);",
  ].join(String.fromCharCode(10)) } },
  { id: "code-ratakan", message: "buatkan fungsi JavaScript bernama persis ratakan yang meratakan array bersarang berapa pun dalamnya menjadi satu array datar, tanpa mengubah array aslinya. Tulis kodenya saja.", expectedIntent: "request", expectedRisk: "biasa", allowCode: true, codeCheck: { symbol: "ratakan", assertions: [
    "assert.deepEqual(ratakan([1, [2, [3, [4]]]]), [1, 2, 3, 4]);",
    "assert.deepEqual(ratakan([]), []);",
    "assert.deepEqual(ratakan([[], [1], []]), [1]);",
    "const asli = [1, [2]]; ratakan(asli); assert.deepEqual(asli, [1, [2]]);",
  ].join(String.fromCharCode(10)) } },
  { id: "code-jam-menit", message: "buatkan fungsi JavaScript bernama persis selisihMenit yang menerima dua string jam format HH:MM dan mengembalikan selisihnya dalam menit, dengan asumsi jam kedua boleh melewati tengah malam. Tulis kodenya saja.", expectedIntent: "request", expectedRisk: "biasa", allowCode: true, codeCheck: { symbol: "selisihMenit", assertions: [
    "assert.equal(selisihMenit('09:00', '10:30'), 90);",
    "assert.equal(selisihMenit('23:30', '00:15'), 45);",
    "assert.equal(selisihMenit('00:00', '00:00'), 0);",
    "assert.equal(selisihMenit('12:00', '11:59'), 1439);",
  ].join(String.fromCharCode(10)) } },
  { id: "code-hitung-kata", message: "buatkan fungsi JavaScript bernama persis hitungKata yang menghitung berapa kali tiap kata muncul dalam sebuah kalimat, mengabaikan beda huruf besar-kecil dan tanda baca, lalu mengembalikan objek biasa. Tulis kodenya saja.", expectedIntent: "request", expectedRisk: "biasa", allowCode: true, codeCheck: { symbol: "hitungKata", assertions: [
    "assert.deepEqual(hitungKata('Ada ada saja'), { ada: 2, saja: 1 });",
    "assert.deepEqual(hitungKata('Halo, halo!'), { halo: 2 });",
    "assert.deepEqual(hitungKata(''), {});",
    "assert.equal(hitungKata('satu dua dua tiga tiga tiga').tiga, 3);",
  ].join(String.fromCharCode(10)) } },
